...
def train():
    with tf.device('/cpu:0'):
        train_text, train_y, train_e1, train_e2, train_pos1, train_pos2, train_relation_truely = \
            data_helpers.load_data_and_labels(FLAGS.train_path, FLAGS.train_features_path, type="train")
    with tf.device('/cpu:0'):
        dev_text, dev_y, dev_e1, dev_e2, dev_pos1, dev_pos2, dev_relation_truely = \
            data_helpers.load_data_and_labels(FLAGS.dev_path, FLAGS.dev_features_path, type="dev")
    with tf.device('/cpu:0'):
        test_text, test_y, test_e1, test_e2, test_pos1, test_pos2, test_relation_truely = \
            ...
            for train_batch in train_batches:
                ...
                if step % FLAGS.evaluate_every == 0:
                    ...
                    for true_label in dev_relation_truely:
                        dev_true_label.append(utils.class2label[true_label])
                    dev_relation_truely_ = np.array(dev_true_label, dtype='int')

                    logger.logging_eval(step, loss, accuracy, predictions, dev_relation_truely_)
                    ...
                    if best_f1 < logger.best_f1:
                        ...
                        with open("./predict_results/test1_50_predict_labels.txt", 'w') as test_pre:
                            for i in predictions_test_list:
                                test_pre.write(utils.label2class[i] + "\n")
                        test_pre.close()
